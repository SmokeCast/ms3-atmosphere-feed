...
La API consulta datos existentes; no ejecuta cargas masivas ni consulta Open-Meteo.
...
- `GET /api/weather?limit=50&skip=0`: `{count, data}`; máximo 500 lecturas.
- `GET /api/weather/city/{city_id}?limit=24`: `{city_id, city_name, count, readings}`.
- `GET /api/weather/latest?city_id=1`: última lectura almacenada de esa ciudad.
- `GET /api/weather/city/{city_id}/summary`: `{city, weather}`; consulta la ciudad
